      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-gray-800 rounded-xl p-4">
          ...
          <div className="h-48 flex items-end gap-2">
            {(data?.hourlyRequests || []).map((r, i) => (<div key={i} className="flex-1 bg-indigo-500 rounded-t" style={{ height: `${Math.min(100, (r.requests / 100) * 100)}%` }}></div>))}
          </div>
        </div>
        ...
        <div className="bg-gray-800 rounded-xl p-4">
          ...
          <div className="space-y-3">
            {(data?.topModels || []).map((m, i) => (<div key={i} className="flex items-center gap-3">
