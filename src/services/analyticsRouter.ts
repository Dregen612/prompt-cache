...
const dailyStore = new Map<string, Map<string, DailyRecord>>(); // key -> dateStr -> record
const modelStore = new Map<string, Map<string, ModelRecord>>(); // key -> model -> record
...
function ensureDay(key: string, dateStr: string): DailyRecord {
  if (!dailyStore.has(key)) dailyStore.set(key, new Map());
...
function ensureModel(key: string, model: string): ModelRecord {
  if (!modelStore.has(key)) modelStore.set(key, new Map());
...
export function recordAnalytics(apiKey: string | undefined, cached: boolean, latency: number, model: string) {
  ...
  for (const d of dayMap.keys()) {
    if (d < daysAgo(MAX_DAYS)) dayMap.delete(d);
  }
}
...
function getRealSummary(key: string) {
...
function getRealTimeline(key: string) {
  const dayMap = dailyStore.get(key) || new Map();
  const days = [];
  for (let i = 29; i >= 0; i--) {
...
function getRealModels(key: string) {
...
function getTierFromKey(apiKey: string | undefined, getKeyTier: (key: string) => Tier | undefined): Tier {
...
export function createAnalyticsRouter(getKeyTier: (key: string) => Tier | undefined): Router {
  const router = Router();
