...
const IMPORTANCE_WEIGHTS: { [key: string]: number } = {
...
export function extractPromptDNA(prompt: string): {
  dna: string;
  keywords: string[];
  complexity: number;
  category: string;
  ...
  const dna = crypto.createHash('sha256').update(dnaInput).digest('hex').slice(0, 16);
...
export function calculateSimilarity(
  dna1: { keywords: string[]; complexity: number; category: string },
  dna2: { keywords: string[]; complexity: number; category: string }
): number {
...
export function findSimilarPrompts(
  targetDNA: ReturnType<typeof extractPromptDNA>,
  cachedEntries: Array<{ prompt: string; dna: ReturnType<typeof extractPromptDNA> }>,
...
export function explainDNA(dna: ReturnType<typeof extractPromptDNA>): string {
